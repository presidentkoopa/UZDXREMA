...
class OptionMenu : Menu
{
	...
	int GetAutoScrollLimit()
	{
		...
		// Mirror Drawer()'s row budget exactly - including the tooltip strip, which
		// shrinks the drawable area. Counting rows against the full screen height
		// would over-count the visible rows and make the credits loop early.
		ScreenArea box;
		GetTooltipArea(box);
		if (!DrawTooltips)
		...
		int visible = MAX((lastrow - y) / rowheight + 1, 1);

		int fixedCount = clamp(mDesc.mScrollTop, 0, mDesc.mItems.Size());
		int scrollableVisible = MAX(visible - fixedCount, 0);
	...
	override void Drawer ()
	{
		...
		int y = mDesc.mPosition;
		// Sub-row smooth offset for the auto-scrolling info/credits pages. It is
		// applied to the drawn row positions only, never to the layout cursor `y`,
		// so LastRow/MaxItems stay in sync with upstream's scroll model.
		...
		for (i = 0; i < mDesc.mItems.Size() && y <= LastRow; i++)
		{
			...
			if (i == mDesc.mScrollTop)
			{
				...
				if (i < 0) i = 0;
				drawOffset = scrollOffset;	// only the scrollable region slides
			}
			...
			int drawY = int(y - drawOffset);

			bool isSelected = mDesc.mSelectedItem == i;
			int cur_indent = mDesc.mItems[i].Draw(mDesc, drawY, indent, isSelected);
			if (cur_indent >= 0 && isSelected && mDesc.mItems[i].Selectable() && mDesc.mItems[i].Visible())
			{
				...
				{
					DrawOptionText(cur_indent + 3 * CleanXfac_1, drawY, OptionMenuSettings.mFontColorSelection, "◄");
				}
			}
