...
class OptionMenuItemSubmenu : OptionMenuItem
{
	...
	// Merged signature: the fork's `cr` (per-item submenu font colour, which
	// feeds mColor) is kept AHEAD of upstream's graycheck trio so that existing
	// 3/4-argument call sites keep their meaning. This must stay in lockstep
	// with CreateOptionMenuItemSubmenu in src/common/menu/menu.cpp (VMValue
	// order) and its declaration in src/common/menu/menu.h.
	...
	{
		// UZDXREMA: mColor is the fork's per-item submenu colour (set from `cr`
		// in Init); it defaults to OptionMenuSettings.mFontColorMore.
...
class OptionMenuItemStaticText : OptionMenuItem
{
	...
	// Merged signature: the fork's `center` is kept AHEAD of upstream's graycheck
	// trio so existing 2/3-argument call sites keep their meaning (menudef.cpp
	// passes `false` to left-align the "---------------" separator, doommenu.cpp
	// passes `true` to centre the "Local IP: ..." item). Must stay in lockstep
	// with CreateOptionMenuItemStaticText in src/common/menu/menu.cpp.
	...
	{
		Super.Init(label, 'None', center, graycheck, graycheckVal, graycheckMode);
...
class OptionMenuItemColorPicker : OptionMenuItem
{
	...
	OptionMenuItemColorPicker Init(
		String label,
		Name command,
		CVar graycheck = null,
		int graycheckVal = 0,
...
{
	OptionMenuItemNumberField Init (
		String label,
		Name command,
		float minimum = 0,
		float maximum = 100,
...
class OptionMenuItemScaleSlider : OptionMenuItemSlider
{
	...
	OptionMenuItemScaleSlider Init(
		String label,
		Name command,
		double min,
		double max,
