...
struct Side native play
{
	...
	// as before.
	native clearscope void SetGlowColor(int pos, color scolor);
	native clearscope void SetGlowHeight(int pos, double height);
...
struct Sector native play
{
	...
	// [BB] The glow setters below are clearscope, and the getters above always
	// were. Glow is RENDER state that happens to be stored on a play struct: it
	// inherited play scope from its address, not from what it does. Nothing in
	// the simulation reads any of it -- every read in the engine is a renderer
	// one (sector_t::CheckSpriteGlow, called only from hw_sprites.cpp and
	// hw_weapon.cpp; sector_t::GetWallGlow, called only from hw_walls.cpp).
	// Level's own SetDarkness/SetGlowWave are clearscope for exactly this
	// reason; these are the same kind of value on a different struct.
	//
	// What it buys: a mod's options menu can re-tint the level WHILE PAUSED.
	// The playsim is frozen with the menu up, so a play-scope setter cannot be
	// reached from UiTick, and every glow slider was dead until you closed the
	// menu -- you could not see the change you were dragging for.
	//
	// The cost, stated plainly: ui code can now write these fields, which in a
	// networked game is a desync door. It is only ajar for values nothing
	// simulates on, and this fork is single-player VR.
	native clearscope void SetGlowHeight(int pos, double height);
	native clearscope void SetGlowColor(int pos, color color);
	...
	native clearscope void SetGlowColorFar(int pos, color color);
	...
	// falloff, intensity.
	native clearscope void SetGlowFalloff(int pos, int falloff);
	native clearscope void SetGlowIntensity(int pos, double intensity);
