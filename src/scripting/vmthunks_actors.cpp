...
DEFINE_FIELD(AActor, HmdPos)
DEFINE_FIELD(AActor, HmdYaw)
...
DEFINE_FIELD(AActor, FingerTouchOff)
DEFINE_FIELD(AActor, GripContextMain)
...
DEFINE_FIELD(AActor, RenderAttachParent)   // [XR] so a mod can attach once instead of every tic -- see actor.h
DEFINE_FIELD(AActor, VRTurnYaw)
DEFINE_FIELD(AActor, LaserTraceTargetMain)
DEFINE_FIELD(AActor, LaserTraceTargetOff)
DEFINE_FIELD(AActor, LaserTraceHitPosMain)
DEFINE_FIELD(AActor, LaserTraceHitPosOff)
...
DEFINE_FIELD(AActor, VoxelOverride)
DEFINE_FIELD(AActor, ForceModelAngles)
DEFINE_FIELD(AActor, FollowBodyMode)
DEFINE_FIELD(AActor, FollowBodyOfs)
DEFINE_FIELD(AActor, HardpointButtons)
DEFINE_FIELD(AActor, OutlineColorA)
DEFINE_FIELD(AActor, OutlineColorB)
DEFINE_FIELD(AActor, OutlineStrength)
DEFINE_FIELD(AActor, OutlineThickness)
DEFINE_FIELD(AActor, OutlineThreshold)
...
// null restores the default (the pawn itself) -- the behaviour of the original port.
DEFINE_ACTION_FUNCTION(AActor, SetVRBodyActor)
{
	PARAM_SELF_PROLOGUE(AActor);
...
// Rig table: role -> joint name on the body's model. bone 'none' removes the role.
DEFINE_ACTION_FUNCTION(AActor, SetVRBodyBone)
{
	PARAM_SELF_PROLOGUE(AActor);
	PARAM_NAME(role);
	PARAM_NAME(bone);
	...
	player->vr_body_rig_gen++;
	return 0;
}

// Finger flex axis in joint-local space for the grip curl (rig-specific).
DEFINE_ACTION_FUNCTION(AActor, SetVRBodyGripAxis)
{
	PARAM_SELF_PROLOGUE(AActor);
...
// Collapse (or restore) a joint and everything under it in the drawn body.
DEFINE_ACTION_FUNCTION(AActor, SetVRBodyHiddenBone)
{
	PARAM_SELF_PROLOGUE(AActor);
	PARAM_NAME(bone);
	PARAM_BOOL(hidden);
	...
	for (unsigned i = 0; i < list.Size(); i++)
	{
		if (list[i] == bone)
		{
			if (!hidden) { list.Delete(i); player->vr_body_rig_gen++; }
			return 0;
		}
	}
	if (hidden) { list.Push(bone); player->vr_body_rig_gen++; }
	return 0;
}

// Pose frame for one hand's fingers, from the body model's own pose clip. frame -1 = bind + curl.
DEFINE_ACTION_FUNCTION(AActor, SetVRBodyHandPose)
{
	PARAM_SELF_PROLOGUE(AActor);
	PARAM_INT(hand);
	PARAM_INT(frame);
	...
	player->vr_body_hand_pose_lerp[hand]  = (float)lerp;
	return 0;
}

// Where one hand IS, in Doom world coordinates: the IK puts the wrist there instead of on the raw
// controller position. valid=false returns that hand to the controller.
DEFINE_ACTION_FUNCTION(AActor, SetVRBodyHandTarget)
{
	PARAM_SELF_PROLOGUE(AActor);
	PARAM_INT(hand);
	PARAM_FLOAT(x); PARAM_FLOAT(y); PARAM_FLOAT(z);
...
// The hand model's drawn axes for one hand (GL world directions of its model +X/+Y/+Z, e.g. from
// Actor.ModelPointToWorld). valid=false returns the wrist orientation to the controller.
DEFINE_ACTION_FUNCTION(AActor, SetVRBodyHandFrame)
{
	PARAM_SELF_PROLOGUE(AActor);
	PARAM_INT(hand);
	PARAM_FLOAT(axx); PARAM_FLOAT(axy); PARAM_FLOAT(axz);
	...
	player->vr_body_hand_frame_ay[hand] = FVector3((float)ayx, (float)ayy, (float)ayz);
	player->vr_body_hand_frame_az[hand] = FVector3((float)azx, (float)azy, (float)azz);
	return 0;
}

// Palm alignment for one hand: rotation taking this body's hand-bone palm frame onto the hand
// model's palm frame (both in their file spaces). Computed offline from the two model files.
DEFINE_ACTION_FUNCTION(AActor, SetVRBodyHandAlign)
{
	PARAM_SELF_PROLOGUE(AActor);
	PARAM_INT(hand);
	...
	q.MakeUnit();
	player->vr_body_hand_align[hand] = q;
	player->vr_body_hand_align_set[hand] = true;
	return 0;
}

// The hand MODEL ACTOR one of the body's hands follows, the joint the wrist lands on, and a fixed
// offset in that model's file space. The renderer reads the actor's live matrix every frame.
// null actor stops following.
DEFINE_ACTION_FUNCTION(AActor, SetVRBodyHandActor)
{
	PARAM_SELF_PROLOGUE(AActor);
	PARAM_INT(hand);
	...
	player->vr_body_hand_offset_model[hand] = FVector3((float)ox, (float)oy, (float)oz);
	return 0;
}

// RENDER ATTACHMENT (actor.h): draw this actor at a joint of another actor's model, from that
// actor's live matrix each frame. offset in the parent model's file space; angles added to the
// parent's facing. 'none' bone = the model origin.
DEFINE_ACTION_FUNCTION(AActor, SetRenderAttachment)
{
	PARAM_SELF_PROLOGUE(AActor);
	PARAM_OBJECT(parent, AActor);
	PARAM_NAME(bone);
	PARAM_FLOAT(ox); PARAM_FLOAT(oy); PARAM_FLOAT(oz);
	PARAM_FLOAT(yaw); PARAM_FLOAT(pitch); PARAM_FLOAT(roll);
	self->RenderAttachParent = (parent != self) ? parent : nullptr;
	self->RenderAttachBone = bone;
	self->RenderAttachOffset = FVector3((float)ox, (float)oy, (float)oz);
	self->RenderAttachAngles = DRotator(DAngle::fromDeg(pitch), DAngle::fromDeg(yaw), DAngle::fromDeg(roll));
	return 0;
}

// Same, but the offset is computed so the actor draws at `worldPos` right now and rides the
// parent from there; yaw is taken relative to the parent's facing.
DEFINE_ACTION_FUNCTION(AActor, SetRenderAttachmentAtWorld)
{
	PARAM_SELF_PROLOGUE(AActor);
	PARAM_OBJECT(parent, AActor);
	PARAM_NAME(bone);
	PARAM_FLOAT(wx); PARAM_FLOAT(wy); PARAM_FLOAT(wz);
	PARAM_FLOAT(worldYaw); PARAM_FLOAT(pitch); PARAM_FLOAT(roll);
	if (parent == nullptr || parent == self) { self->RenderAttachParent = nullptr; ACTION_RETURN_BOOL(false); }
	FVector3 off;
	if (!VR_ModelWorldToJointOffset(parent, bone, DVector3(wx, wy, wz), off)) ACTION_RETURN_BOOL(false);
	self->RenderAttachParent = parent;
	self->RenderAttachBone = bone;
	self->RenderAttachOffset = off;
	self->RenderAttachAngles = DRotator(DAngle::fromDeg(pitch), DAngle::fromDeg(worldYaw - VR_ActorFacing(parent)), DAngle::fromDeg(roll));
	ACTION_RETURN_BOOL(true);
}

DEFINE_ACTION_FUNCTION(AActor, ClearRenderAttachment)
{
	PARAM_SELF_PROLOGUE(AActor);
	self->RenderAttachParent = nullptr;
	self->RenderAttachBone = NAME_None;
	return 0;
}

// Forget the whole rig description (table, hidden bones, axis, hand poses, hand targets).
DEFINE_ACTION_FUNCTION(AActor, ClearVRBodyRig)
{
	...
	player->vr_body_grip_axis = FVector3(1.f, 0.f, 0.f);
	player->vr_body_neck_z = 0.f;
	for (int h = 0; h < 2; h++)
	{
		player->vr_body_hand_pose_frame[h] = player->vr_body_hand_pose_next[h] = -1;
		player->vr_body_hand_pose_lerp[h] = 0.f;
		player->vr_body_hand_target_valid[h] = false;
		player->vr_body_hand_frame_valid[h] = false;
		player->vr_body_hand_align_set[h] = false;
		player->vr_body_hand_actor[h] = nullptr;
		player->vr_body_hand_palm_bone[h] = NAME_None;
	}
	player->vr_body_rig_gen++;
	return 0;
}
