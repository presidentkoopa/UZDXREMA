...
// These two are GENERATED by lemon from common/scripting/frontend/zcc-parse.lemon
// into ${CMAKE_CURRENT_BINARY_DIR} (i.e. <build>/src) at build time - see the
// add_custom_command in src/CMakeLists.txt. They must NEVER be checked in.
//
// The UZDXREMA fork used to keep a snapshot of the generated zcc-parse.c/.h/.out in
// src/scripting/zscript/. That directory sits far earlier on the MSVC include path
// than the build directory, so the stale snapshot silently shadowed the freshly
// generated pair. After the 5.0.0-rc.2 merge the grammar gained the LTEQGT ('<=>')
// and NOROLLBACK terminals, the snapshot did not, and ZCC_LTEQGT / ZCC_NOROLLBACK
// below stopped resolving. The snapshot has been removed; do not re-add it.
#include "zcc-parse.h"
#include "zcc-parse.c"
