...
{
	// UZDXREMA: parse VERSIONSTR, not GIT_DESCRIPTION.
	//
	// Upstream derives the running version from `git describe`, which assumes
	// the nearest tag is always a release version. That holds for UZDoom's own
	// repository; it does not hold for a fork, whose tags are working anchors
	// with names like `pre-worldpanel-backup`. VersionInfo cannot parse such a
	// tag, so the engine concluded it was running version 0.0.0 -- which made
	// the asserts below fire on a debug build, and made ReleasePage treat the
	// real 5.0.0 release notes as an unreleased future version and silently
	// skip them (releasepage.cpp, the `> GetCurrentVersionForUpdater()` guard),
	// falling through to print 4.14.3's notes on a 5.0.0 build.
	//
	// VERSIONSTR is the authoritative engine version and is what every other
	// version comparison in the tree uses, so parse that instead. The git
	// description is still what GetVersionString() reports, so the banner and
	// the crash logs keep their exact commit provenance.
