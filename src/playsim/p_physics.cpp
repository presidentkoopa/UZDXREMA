...
//
// P_PhysicsFrame() is called unconditionally from D_DoomLoop, so without this
// the whole module runs every frame whether or not anything uses it: two hand
// bodies built per level, a solver iterating an empty list, and a report line in
// the log once a second forever.
//
// That is not a hypothetical cost -- it is the reason the log has to be waded
// through to find anything. Off by default, so the engine is exactly as it was
// until something asks for physics.
//
// Everything downstream of here is intact. Turning this on restores the module
// whole; it does not need rebuilding.
CVAR(Bool, vr_physics, false, CVAR_ARCHIVE | CVAR_GLOBALCONFIG)

// Was true, which meant a report line every second in every session forever.
CVAR(Bool, vr_physics_debug, false, CVAR_ARCHIVE | CVAR_GLOBALCONFIG)
...
// TWO-HANDED HOLDS. On by default -- a feature built and shipped switched off
// is a feature nobody has.
CVAR(Bool, vr_physics_support, true, CVAR_ARCHIVE | CVAR_GLOBALCONFIG)
// How much of the aim the support hand gets. 1 = it points the gun; 0 = it is
// along for the ride and only the firing hand aims.
CVAR(Float, vr_physics_support_strength, 1.0f, CVAR_ARCHIVE | CVAR_GLOBALCONFIG)
...
// How hard a throw actually leaves the hand, as a multiplier on the measured
// peak. 1.0 is "exactly as fast as the fastest moment of your swing".
//
// The peak-of-180ms rule is right for a real throw and wrong for putting
// something down: any tracking jitter inside that window becomes the release
// velocity, so a gun set down carefully still leaves at the speed of the worst
// single sample in the last fifth of a second. Scaling the peak lets that be
// dialled to taste without going back to release-instant sampling, which is the
// thing that made throws dribble.
CVAR(Float, vr_physics_throwscale, 0.45f, CVAR_ARCHIVE | CVAR_GLOBALCONFIG)

// Below this the peak is ignored entirely and the object simply leaves at
// whatever the hand is doing now. This is the difference between a throw and
// letting go: under half a metre per second is not a throw, it is a release,
// and it should not inherit a spike from earlier in the window.
CVAR(Float, vr_physics_throwmin, 0.9f, CVAR_ARCHIVE | CVAR_GLOBALCONFIG)
...
struct Quat
{
	...
	// a * b, applying b then a. Written out longhand at three sites before this
	// existed, which is three chances to transpose a sign.
	...
	// The shortest rotation taking direction a onto direction b. Both must be
	// unit. A SWING with no twist -- it turns the gun to point where the support
	// hand is without rolling it, which is what a hand on a forend does.
	static Quat Swing(const FVector3 &a, const FVector3 &b)
	{
		const float d = a | b;
		if (d > 0.999999f) return Identity();
		if (d < -0.999999f)
		{
			// Exactly opposed: every axis perpendicular to a is a valid answer,
			// so pick one that is definitely not parallel to it.
			// Cross() is declared 600 lines below this struct, so it is written
			// out here rather than reordering the file around one call.
			FVector3 ax(0.f * a.Z - 0.f * a.Y, 0.f * a.X - 1.f * a.Z, 1.f * a.Y - 0.f * a.X);
			if (ax.LengthSquared() < 1e-6f)
				ax = FVector3(1.f * a.Z - 0.f * a.Y, 0.f * a.X - 0.f * a.Z, 0.f * a.Y - 1.f * a.X);
			ax.MakeUnit();
			return { ax.X, ax.Y, ax.Z, 0.f };
		}
		const FVector3 ax(a.Y*b.Z - a.Z*b.Y, a.Z*b.X - a.X*b.Z, a.X*b.Y - a.Y*b.X);
		const float  s = sqrtf((1.f + d) * 2.f);
		Quat q{ ax.X / s, ax.Y / s, ax.Z / s, s * 0.5f };
		q.Normalize();
		return q;
	}
...
struct PhysBody
{
	...
	// A SECOND HAND ON THE SAME OBJECT.
	//
	// -1 for none. supportLocal is the point the support hand took hold of, in
	// the body's own frame, captured ONCE when the support begins -- never
	// re-captured while it lasts, because a point re-measured every step is a
	// point that never disagrees with where the hand is, and the whole effect
	// comes from that disagreement being corrected.
	//
	// This lives in the solver and not in script for the reason spelled out in
	// m9phys.zs: a held body is carried at physics rate, so anything script
	// writes to its transform at 35 Hz is overwritten before it is ever drawn.
	int      supportHand = -1;
	FVector3 supportLocal = FVector3(0, 0, 0);
...
void UpdateHands(float dt)
{
	...
	for (unsigned i = 0; i < g_bodies.Size(); i++)
	{
		...
		// TWO-HANDED.
		//
		// The firing hand keeps position and roll; the support hand decides
		// where the thing POINTS. That division is what makes a second hand feel
		// like leverage rather than like a second grab: you brace a forend and
		// the muzzle goes where you put it, and the gun still rolls with your
		// firing wrist.
		//
		// Implemented as a swing from where the support point currently is to
		// where the support hand actually is. Swing only -- no twist -- so it
		// cannot fight the firing hand for roll.
		const PhysBody *sup = nullptr;
		if (h.supportHand >= 0 && vr_physics_support)
		{
			for (unsigned k = 0; k < g_bodies.Size(); k++)
				if (g_bodies[k].handIndex == h.supportHand) { sup = &g_bodies[k]; break; }
		}
		if (sup != nullptr)
		{
			const FVector3 curV = q.Rotate(h.supportLocal);
			const FVector3 wantV = sup->pos - h.pos;
			const float curLen = curV.Length(), wantLen = wantV.Length();
			// Below a centimetre the direction is noise, not an aim.
			if (curLen > 0.01f && wantLen > 0.01f)
			{
				Quat sw = Quat::Swing(curV / curLen, wantV / wantLen);
				// Strength, as a blend toward identity. 1 is "the support hand
				// aims it"; lower leaves the firing hand more of the say, which
				// is a taste question and therefore a slider.
				float k = clamp((float)vr_physics_support_strength, 0.f, 1.f);
				if (k < 0.999f)
				{
					sw.x *= k; sw.y *= k; sw.z *= k;
					sw.w = sw.w * k + (1.f - k);
					sw.Normalize();
				}
				q = Quat::Mul(sw, q);
...
void P_PhysicsFrame()
{
	// Off means OFF -- no stepping, no hand bodies, no reporting, nothing.
	// Bodies are cleared rather than left frozen mid-air if it is switched off
	// while something is simulating.
	if (!vr_physics)
	{
		if (g_bodies.Size() > 0) g_bodies.Clear();
		return;
	}
...
static void PhysicsGrab(AActor *self, int hand)
{
	...
	b->heldByHand = hand;
	b->supportHand = -1;   // a fresh grab starts one-handed
	b->kinematic = true;
	b->asleep = false;
	b->sleepTimer = 0.f;
	b->restReported = false;
...
// Put a second hand on this body, at a world point on it, or clear it with a
// negative hand.
//
// SCRIPT DECIDES WHEN, THE SOLVER DECIDES HOW. Whether your off hand is on the
// gun is a proximity question against the gun's own collision volume, which
// script already answers for grabbing; what that does to the transform has to
// happen at physics rate or it is overwritten before it is drawn.
static void PhysicsSetSupport(AActor *self, int hand, double wx, double wy, double wz)
{
	if (self == nullptr) return;
	PhysBody *b = FindBody(self);
	if (b == nullptr)
	{
		Printf("\x1b[33m[PHYS] support refused: %s has no physics body\n",
			self->GetClass()->TypeName.GetChars());
		return;
	}
	...
	if (b->heldByHand < 0)
	{
		// Supporting something nobody is holding is meaningless, and silently
		// accepting it would leave a body that aims itself at a hand forever.
		Printf("\x1b[33m[PHYS] support refused: %s is not being held\n",
			self->GetClass()->TypeName.GetChars());
		return;
	}
	if (hand == b->heldByHand)
	{
		Printf("\x1b[33m[PHYS] support refused: hand %d is already the holding hand\n", hand);
		return;
	}

	// Captured ONCE, on the transition. Re-measuring it every tic would make
	// the correction identically zero and the whole thing a no-op that looks
	// wired up.
	if (b->supportHand != hand)
	{
		const FVector3 w(MapToM(wx), MapToM(wy), MapToM(wz));
		b->supportLocal = b->rot.Inverse().Rotate(w - b->pos);
		b->supportHand = hand;
		if (vr_physics_debug)
			Printf("[PHYS] %s supported by hand %d at local (%.3f %.3f %.3f)\n",
				self->GetClass()->TypeName.GetChars(), hand,
				b->supportLocal.X, b->supportLocal.Y, b->supportLocal.Z);
	}
}

DEFINE_ACTION_FUNCTION_NATIVE(AActor, PhysicsSetSupport, PhysicsSetSupport)
{
	PARAM_SELF_PROLOGUE(AActor);
	PARAM_INT(hand);
	PARAM_FLOAT(wx);
	PARAM_FLOAT(wy);
	PARAM_FLOAT(wz);
	PhysicsSetSupport(self, hand, wx, wy, wz);
	return 0;
}

static int PhysicsGetSupport(AActor *self)
{
	if (self == nullptr) return -1;
	PhysBody *b = FindBody(self);
	return b ? b->supportHand : -1;
}

DEFINE_ACTION_FUNCTION_NATIVE(AActor, PhysicsGetSupport, PhysicsGetSupport)
{
	PARAM_SELF_PROLOGUE(AActor);
	ACTION_RETURN_INT(PhysicsGetSupport(self));
}
...
{
	// Letting go with the firing hand ends the hold entirely -- a gun cannot be
	// held by a support hand alone, and leaving the field set would have the
	// next grab inherit a second hand that is not there.
	if (self) { PhysBody *sb = FindBody(self); if (sb) sb->supportHand = -1; }
	if (self == nullptr) return;
	PhysBody *b = FindBody(self);
	if (b == nullptr) return;
	...
	if (hand >= 0 && hand <= 1)
	{
		...
		//
		// AND only if it is fast enough to be a throw at all. Without the floor,
		// a careful release still inherits the fastest jitter sample from the
		// last 180ms, which is why setting a gun down sent it across the room.
		if (best != nullptr && bestSpeed > b->vel.Length() && bestSpeed >= vr_physics_throwmin)
		{
			b->vel = bestVel * (float)vr_physics_throwscale;
			b->angVel = best->angVel * (float)vr_physics_throwscale;
		}
	}
