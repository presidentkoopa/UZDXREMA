...
//==========================================================================
//
// [STAMP] stamp -- fire one surface stamp at whatever you are looking at.
//
// A test handle, and the whole primitive demonstrated in one line: no actor, no
// light, no decal, no mod. Traces the player's aim to the first surface and
// publishes a stamp there.
//
//   stamp [shape] [radius] [life-tics] [texture] [tex-strength]
//
// Shape and texture numbers are the STAMP_* and STAMPTEX_* lists at the top of
// func_surfacestamps.fp. Colour is deliberately not a parameter and rolls per
// call: the point of the command is to see the SHAPE, and typing three more
// numbers to check a ring is worse than getting a different colour each time.
//
//==========================================================================
...
CCMD (stamp)
{
	...
	{
		Printf("stamp: no player in a level\n");
		return;
	}

	const int    shape  = (argv.argc() > 1) ? atoi(argv[1]) : 3;      // ring
	const double radius = (argv.argc() > 2) ? atof(argv[2]) : 96.0;
	const int    life   = (argv.argc() > 3) ? atoi(argv[3]) : 35;     // one second
	const int    tex    = (argv.argc() > 4) ? atoi(argv[4]) : 0;
	const double texStr = (argv.argc() > 5) ? atof(argv[5]) : 0.6;

	// A stamp wants the SURFACE that was hit, which is exactly what a line
	// trace returns. Fired from eye height so looking down hits the floor in
	// front rather than the one underfoot.
	FLineTraceData trace;
	P_LineTrace(mo, mo->Angles.Yaw, 8192.0, mo->Angles.Pitch, 0,
		mo->Height * 0.75, 0.0, 0.0, &trace);

	if (trace.HitType == TRACE_HitNone)
	{
		Printf("stamp: nothing in front of you\n");
		return;
	}

	// The trace direction doubles as the axis, so a gouge lies along the way
	// the shot travelled rather than at some fixed angle.
	static int stampHue = 0;
	stampHue = (stampHue + 67) % 360;
	const double h = stampHue * M_PI / 180.0;
	const PalEntry col = PalEntry(255,
		(uint8_t)(128 + 127 * cos(h)),
		(uint8_t)(128 + 127 * cos(h + 2.0944)),
		(uint8_t)(128 + 127 * cos(h + 4.1888)));

	mo->Level->SpawnSurfaceStamp(shape, trace.HitLocation, radius, col, life,
		trace.HitDir, tex, texStr);

	Printf("stamp: shape %d r %.0f %d tics tex %d at (%.0f %.0f %.0f)\n",
		shape, radius, life, tex,
