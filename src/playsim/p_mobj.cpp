...
void AActor::Serialize(FSerializer &arc)
{
	...
	arc
		...
		("modelData", modelData)
		A("VoxelOverride", VoxelOverride)
		A("ForceModelAngles", ForceModelAngles)
		A("HardpointButtons", HardpointButtons)
			A("FollowBodyMode", FollowBodyMode)
			A("FollowBodyYaw", FollowBodyYaw)
			A("FollowBodyOfs", FollowBodyOfs)
			...
			A("FollowActorOfsInModel", FollowActorOfsInModel)
			A("FollowActorOfsCVar", FollowActorOfsCVar)
			A("FollowActorOfsCVar2", FollowActorOfsCVar2)
			A("OutlineColorA", OutlineColorA)
			A("OutlineColorB", OutlineColorB)
			A("OutlineStrength", OutlineStrength)
			A("OutlineThickness", OutlineThickness)
			A("OutlineThreshold", OutlineThreshold)
			...
			A("ScaleCVarUnit", ScaleCVarUnit)
			A("PulseHz", PulseHz)
			A("PulseDepth", PulseDepth)
			A("TintColor", TintColor)
			A("ScaleAxes", ScaleAxes)
...
// [UZDXREMA] Deliberately non-static: the VR device layer (vk_openxrdevice.cpp,
// gl_openxrdevice.cpp, gl_openvr.cpp) calls this directly for roomscale locomotion.
...
//
	...
	// clip movement
	//
	// [UZDXREMA] + 2 is the VR floor-contact slack; matches the sibling checks
	// in this file (friction check, NOGRAVITY player branch, AActor::FallAndSink).
	if (mo->Z() <= mo->floorz + 2)
	{
		// Hit the floor
							  ...
							  bool nofreeaim, bool noautoaim, int aimflags)
{
	...
	// [UZDXREMA] Offhand missiles fire on CHAN_OFFWEAPON with CHANF_OVERLAP so they
	// don't stomp the mainhand weapon channel.
