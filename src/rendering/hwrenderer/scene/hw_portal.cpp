...
// [UZDXREMA] Per-eye portal budget. Consumed by hw_drawinfo.cpp:
// "if (gl_max_portals > -1 && portalsPerEye >= gl_max_portals) return;"
CVAR(Int, gl_max_portals, -1, CVAR_ARCHIVE);

// r_portal_recursions comes from hw_cvars.h (was r_mirror_recursions before 5.0.0-rc.2).
...
bool FPortalSceneState::RenderFirstSkyPortal(int recursion, HWDrawInfo *outer_di, FRenderState &state)
{
	...
	if (best)
	{
		...
		// The sky projection swap (mono and, for VR, the per-eye multiview matrices) used to
		// live here in UZDXREMA. Upstream moved it into RenderPortal() below, which also covers
		// the EndFrame() path, so it is re-seated there rather than duplicated here.
...
{
	VSMatrix tempmatrix;
	VSMatrix tempmatrixLeft, tempmatrixRight;
	const bool isSkyPortal = (p->GetHWPortalType() == HWP_SKY) || (p->GetHWPortalType() == HWP_SKYBOX);
	const bool swapSkyProjection = outer_di->Viewpoint.bDoOrtho && isSkyPortal;
	// [UZDXREMA] In stereo/multiview rendering the mono VPUniforms are not what the eyes read.
	// The per-eye viewpoints need the very same perspective-projection substitution, otherwise
	// the sky/skybox gets drawn through the orthographic matrix in VR.
	const bool swapMultiviewSkyProjection = swapSkyProjection && outer_di->HasMultiviewViewpoints && outer_di->HasMultiviewProjectionMatrix2;
	if (swapSkyProjection)
	{
		...
		if (swapMultiviewSkyProjection)
		{
			tempmatrixLeft = outer_di->MultiviewVPUniforms[0].mProjectionMatrix;
	...
	if (swapSkyProjection)
	{
		...
		if (swapMultiviewSkyProjection)
		{
			outer_di->MultiviewVPUniforms[0].mProjectionMatrix = tempmatrixLeft;
...
void HWPortal::SetupStencil(HWDrawInfo *di, FRenderState &state, bool usestencil)
{
	...
	rendered_portals++;
	portalsPerEye++;	// [UZDXREMA] per-eye portal budget, see gl_max_portals
...
bool HWSkyboxPortal::Setup(HWDrawInfo *di, FRenderState &rstate, Clipper *clipper)
{
	...
	// [UZDXREMA] Strip the per-eye positional offset so a skybox reads as infinitely distant
	// in stereo instead of as a small room sitting right in front of the player.
...
bool HWPlaneMirrorPortal::Setup(HWDrawInfo *di, FRenderState &rstate, Clipper *clipper)
{
	...
	di->SetupView(rstate, vp.Pos.X, vp.Pos.Y, vp.Pos.Z, !!(state->MirrorFlag & 1), !!(state->PlaneMirrorFlag & 1));
	vp.ViewVector3D.Z = - vp.ViewVector3D.Z;
	// NOTE: vp.OffPos.Z is already mirrored above, before SetupView(). Do not mirror it a
	// second time here (that was the old UZDXREMA placement) or it cancels back out.
	SetupCoverage(di);
	ClearClipper(di, clipper);
