...
void NetUpdate(int tics)
{
	...
	{
		// [UZDXREMA] In VR the runtime owns the input pump; don't double-drive it here.
		{
...
bool D_CheckNetGame()
{
	if (!I_InitNetwork())
	{
		// [UZDXREMA] Cancelling the VR net-wait lobby falls back to single player
		// instead of aborting boot entirely. I_ConsumeCancelledNetWaitBoot()
		// restores the single player boot state; fall through so the rest of the
		// normal setup below still runs against it.
...
void TryRunTics()
{
	...
	// [UZDXREMA] In VR the HMD compositor owns frame pacing, so never block here.
	auto vrmode = VRMode::GetVRMode();
	if (vrmode != nullptr && vrmode->IsVR())
		doWait = false;
	...
	if (vid_dontdowait && (vid_maxfps > 0 || vid_vsync))
		doWait = false;
	// [UZDXREMA] ...and keep rendering for the headset when the desktop window loses focus.
	if (!netgame && !AppActive && vid_lowerinbackground && (vrmode == nullptr || !vrmode->IsVR()))
		doWait = true;
...
void Net_DoCommand(int cmd, TArrayView<uint8_t>& stream, int player)
{
	...
	case DEM_WARPCHEAT:
		{
			// [UZDXREMA] Full-range coordinates + optional telefrag for roomscale teleport.
			int x = ReadInt32(stream);
			int y = ReadInt32(stream);
			int z = ReadInt32(stream);
			bool telefrag = !!ReadInt8(stream);
			auto mo = players[player].mo;
			if (mo != nullptr)
			{
	...
	case DEM_ZSC_CMD:
		{
			...
			// [UZDXREMA] VR weapon-hand net commands.
